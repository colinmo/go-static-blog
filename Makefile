...
build-windows-windows:
	set GOOS=windows&&set GOARCH=amd64&&cd f:\Dropbox\swap\golang\vonblog\src&&go build -ldflags "-w -s"
	
build-linux-windows:
	set GOOS=linux&&set GOARCH=amd64&&cd f:\Dropbox\swap\golang\vonblog\src&&go build -ldflags "-w -s"
